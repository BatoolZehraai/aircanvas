...
# import Windows sound library (optional feature)
try:
    import winsound
        ...
        while not self.stop_event.is_set():
            if AUDIO_AVAILABLE and self.active:
                try:
                    ...
                    freq = max(100, min(freq, 800)) # Clamp frequency
...
class HandSystem:
    ...
    def draw_sci_fi_hud(self, img, points, pinch_dist):
        ...
        fill = max(0.0, fill)  # Ensure fill is between 0 and 1
    ...
    def draw(self, img, hover_pt):
        ...
        for i in range(num_colors):
            ...
            cv2.ellipse(img, (cx, cy), 
                        ...
                        radius + shift + (thickness//2)), 0, start_ang, end_ang, color, thickness)
...
def main():
    ...
    while True:
        ...
        is_drawing, velocity = False, 0

        if points:
            idx_tip, thm_tip = points[8], points[4]
            cx, cy = idx_tip
            
            if smooth_x == 0: smooth_x, smooth_y = cx, cy
            ...
            if hover_idx != -1 and dist < Config.PINCH_THRESHOLD:
                ...
                cv2.line(canvas, (smooth_x, smooth_y), (cx, cy), current_color, Config.BRUSH_SIZE)
                cv2.circle(canvas, (cx, cy), Config.BRUSH_SIZE // 2, current_color, -1)
